...
abstract contract GovernorSettableFixedQuorum is Governor {
  ...
  Checkpoints.Trace224 internal quorumCheckpoints;
  ...
  function quorum(uint256 _voteStart) public view override returns (uint256) {
    return quorumCheckpoints.upperLookup(SafeCast.toUint32(_voteStart));
  }
  ...
  function _setQuorum(uint224 _amount) internal {
    ...
    quorumCheckpoints.push(SafeCast.toUint32(block.timestamp), _amount);
  }
}
