...
describe("ZkSocialGovernor", function () {
  let protocolGovernor: ZkSocialGovernor;
  ...
  before("Deploy Governor", async function () {
    ...
    protocolGovernor = (await deployer.deploy(ZkSocialGovernor, [
      name,
      mockTokenAddr,
      mockTimelockAddr,
      votingDelay,
  ...
  describe("Constructor", function () {
    it("Set paramters correctly", async function () {
      ...
      expect(await protocolGovernor.guardian()).to.equal(mockGuardian);
    });
  });
});
