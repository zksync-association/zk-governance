...
contract ProtocolUpgradeHandler is IProtocolUpgradeHandler, Initializable {
  ...
  /// execution post-guardians approval. If the Security Council approves, the upgrade can proceed immediately;
  /// otherwise,
  ...
  /// @notice Checks that the message sender is an active Security Council or the protocol is frozen but freeze period
  /// expired.
  ...
  function approveUpgradeSecurityCouncil(bytes32 _id) external onlySecurityCouncil {
    UpgradeState upgState = upgradeState(_id);
    require(
      upgState == UpgradeState.Waiting, "Upgrade with this id is not waiting for the approval from Security Council"
    );
  ...
  function _execute(Call[] calldata _calls) internal {
    for (uint256 i = 0; i < _calls.length; ++i) {
      if (_calls[i].data.length > 0) {
        require(_calls[i].target.code.length > 0, "Target must be a smart contract if the calldata is not empty");
      }
