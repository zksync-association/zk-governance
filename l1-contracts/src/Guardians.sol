...
/// @dev Temporary protector of the values of ZKsync. They can approve upgrade changes proposed by the Token Assembly,
/// propose & cancel L2 proposals as well as extend the legal veto period of L1 upgrade proposals through the
...
contract Guardians is IGuardians, Multisig, EIP712 {
  ...
  /// @dev Tracks the unique identifier used in the last `cancelL2GovernorProposal`/`proposeL2GovernorProposal` to
  /// ensure replay attack protection.
  ...
  function proposeL2GovernorProposal(
    ...
    bytes memory proposeCalldata = abi.encodeCall(
      IL2Governor.propose, (_l2Proposal.targets, _l2Proposal.values, _l2Proposal.calldatas, _l2Proposal.description)
    );
    BRIDGE_HUB.requestL2TransactionDirect{value: _txRequest.txMintValue}(
      L2TransactionRequestDirect({
        chainId: ERA_CHAIN_ID,
